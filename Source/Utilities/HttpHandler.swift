...
final class HttpHandler {
	...
	init(using session: URLSessionStandard = URLSession.defaultConfig()) {
	...
	private func configureRequest(with spec: HttpRequestSpec) -> URLRequest? {
		...
		guard let targetURL = URL(string: spec.url.description) else {
			return nil
		}
		...
		case (true, .get):
			...
			var urlComponents = URLComponents(string: spec.url.description)
